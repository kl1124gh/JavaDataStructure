...
public class SLList<T> extends AbstractQueue<T> {
	//extend: the single linked list has all the features in AbstractQueue  
 class Node {
  T x;
  Node next;
 }
...
/*	 Queue<Location> qlocs = new SLList<Location>(); //create a linked queue of location
	 double x = 0.0;
	 double y = 0.0;
	 for(int i = 0; i < 100;i++) {
		 x = (double) getRandomInt(-100,100);
		 y = (double) getRandomInt(-100,100);
	 }*/
	  //first in first out
  Queue<Integer> q = new SLList<Integer>();// create a linked queue of integer
