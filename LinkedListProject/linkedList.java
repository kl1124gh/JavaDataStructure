...
public class linkedList {  
  ...
  { // comment out the return statement below and add your code 
  	...
  	double totalp  = 0;
  	double totalg = 0;
  	SLList<ProduceItem> a = anOrder.getX();
  	SLList<GenericItem> b = anOrder.getY();
  	int length1 = a.size();
  	int length2 = b.size();
  	for ( int i = 0;i< length1;i++) {
  		totalp+=(a.pop()).getPrice();
  		
  	}
  	for ( int j = 0; j < length2;j++) {
  		totalg+=(b.pop()).getPrice();
  	}
    ...
    { // comment out the return statement below and add your own code 
      //return null;
    	...
    	for ( int i = 0; i < orderSize;i++) {
    		genericItem.add(gItem[i]);
    		produceItem.add(pItem[i]);
    	}
